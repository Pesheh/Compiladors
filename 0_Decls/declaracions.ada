package decls is
  ...
  -- Aquests 2 serien els que teoricament hauriem d'emprar però per simplicitat 
  -- no ho feim
  -- 1Byte x char
...
package body decls.d_c3a is
  ...
  begin
    ...
    if tp(np).tp = std then
      ...
      -- si portessim a terme una etapa d'optimitzacio, aixo hauria de 
      -- calcular-se un cop hagues acabat l'esmentada etapa (es absurd 
      -- calcular una ocupacio que canviara). però com que no es el cas, 
      -- ho feim aquí
  ...
  procedure nova_var_const(nv: in out num_var; tv: in out tvariables; 
                           val: in valor; tsb: in tipus_subjacent; 
                           t: out num_var) is
  begin
    nv:= nv+1;
  ...
  procedure nou_arg(nv: in out num_var; tv: in out tvariables; 
                    tp: in out tprocediments; np: in num_proc; 
                    offset: in out despl; t: out num_var) is
  begin
    nv:= nv+1;
  ...
  procedure nou_proc(np: in out num_proc; tp: in out tprocediments; 
                     e: in num_etiq; prof: in profunditat; nparam: in natural; 
                     p: out num_proc) is
  begin
    np:= np+1;
  ...
  procedure act_proc_args(tp: in out tprocediments; np: in num_proc; 
                          nargs: in natural) is
  begin
    tp(np).nparam:= nargs;
  end act_proc_args;
  ...
  function Value(t: in tinstruccio; a: in num_etiq; b,c: in num_var) 
  return instr_3a is
  begin
  ...
  begin
    if i3a.b = null_nv then
      case i3a.d is
        when comu =>
          return tinstruccio'Image(i3a.t)&""
                 &num_var'Image(i3a.nv)&" - -";
        when proc =>
          return tinstruccio'Image(i3a.t)&""
                 &num_proc'Image(i3a.np)&" - -";
        when etiq =>
          return tinstruccio'Image(i3a.t)&""
                 &num_etiq'Image(i3a.ne)&" - -";
      end case;
      ...
      case i3a.d is
        when comu =>
          return tinstruccio'Image(i3a.t)&""
                 &num_var'Image(i3a.nv)&""
                 &num_var'Image(i3a.b)&""&num_var'Image(i3a.c);
        when proc =>
          return tinstruccio'Image(i3a.t)&""
                 &num_proc'Image(i3a.np)&""
                 &num_var'Image(i3a.b)&""&num_var'Image(i3a.c);
  ...
  begin
    case i3a_b.t is
      when pmb | call | rtn =>
        return (proc, i3a_b.t, np=> num_proc(i3a_b.a), 
                b=> num_var(i3a_b.b), c=> num_var(i3a_b.c));
      when etiq | go_to | ieq_goto =>
        return (etiq, i3a_b.t, ne=> num_etiq(i3a_b.a), 
                b=> num_var(i3a_b.b), c=> num_var(i3a_b.c));
      when others =>
         return (comu, i3a_b.t, nv=> num_var(i3a_b.a), 
                 b=> num_var(i3a_b.b), c=> num_var(i3a_b.c));
    end case;
  end To_i3a;
  ...
  -- Es l'unica manera de mantenir la privacitat del tipus variable. Si 
  -- escau aplicar aquest esquema als camps necessaris a posteriori 
  -- d'aquest tipus
  function consulta_val_const(tv: in tvariables; nv: in num_var) 
  return valor is
  begin
  ...
  function consulta_tsb_const(tv: in tvariables; nv: in num_var) 
  return tipus_subjacent is
  begin
    return tv(nv).all.tsb;
  end consulta_tsb_const;


  function consulta_np_var(tv: in tvariables; nv: in num_var) 
  return num_proc is
  begin
    return tv(nv).all.np;
  end consulta_np_var;
  ...
  function consulta_desp_var(tv: in tvariables; nv: in num_var) 
  return despl is
  begin
    return tv(nv).all.desp;
  end consulta_desp_var;

  function consulta_ocup_var(tv: in tvariables; nv: in num_var) 
  return despl is
  begin
  ...
  function consulta_tproc(tp: in tprocediments; np: in num_proc) 
  return tproc is
  begin
    return tp(np).tp;
  end consulta_tproc;
  ...
  function consulta_prof_proc(tp: in tprocediments; np: in num_proc) 
  return profunditat is
  begin
    return tp(np).prof;
  end consulta_prof_proc;


  function consulta_ocup_proc(tp: in tprocediments; np: in num_proc) 
  return despl is
  begin
    if tp(np).tp = std then
      return 0;
  ...
  function consulta_etiq_proc(tp: in tprocediments; np: in num_proc) 
  return num_etiq is
  begin
    if tp(np).tp = std then
      return null_ne;
  ...
  function consulta_nom_proc(tp: in tprocediments; np: in num_proc; 
                             tn: in tnoms) return String is
  begin
    if tp(np).tp = std then
  ...
  function consulta_nparam_proc(tp: in tprocediments; np: in num_proc) 
  return natural is
  begin
    return tp(np).nparam;
  end consulta_nparam_proc;

  procedure actualitza_ocupvl_proc(tp: in out tprocediments; np: in num_proc; 
                                   ocup: in despl) is
  begin
    tp(np).ocup_vl:=ocup;
  end actualitza_ocupvl_proc;

  procedure actualitza_desp_var(tv: in out tvariables; nv: in num_var; 
                                desp: in despl) is
  begin
    tv(nv).all.desp:=desp;
...
package decls.d_c3a is
  ...
  procedure nova_var(nv: in out num_var; tv: in out tvariables; 
  tp: in out tprocediments; np: in num_proc; ocup: in despl; 
  t: out num_var);
  procedure nova_var_const(nv: in out num_var; tv: in out tvariables; 
  val: in valor; tsb: in tipus_subjacent; t: out num_var);
  procedure nou_arg(nv: in out num_var; tv: in out tvariables; 
  tp: in out tprocediments; np: in num_proc; offset: in out despl; 
  t: out num_var);
  procedure nou_proc(np: in out num_proc; tp: in out tprocediments; 
  e: in num_etiq; prof: in profunditat; nparam: in natural; p: out num_proc);
  procedure nou_proc_std(np: in out num_proc; tp: in out tprocediments; 
  id: in id_nom; prof: in profunditat; nparam: in natural; p: out num_proc);
...
private
  ...
  -- Aquest es el tipus d'instruccions escrites al fitxer binári ja que 
  -- simplifiquen molt la seua gestió
...
package body decls.d_tnoms is
  ...
  function equal(nom: in string; tn: in tnoms; p: in id_nom) return boolean 
  is
...
package body decls.d_tsimbols is
  ...
  procedure put_camp(ts: in out tsimbols; idr,idc: in id_nom; dc: in descripcio; 
                     error: out boolean) is
    td: tdescripcio renames ts.td;
    te: texpansio renames ts.te;
    tb: tblocks renames ts.tb;
  ...
  procedure put_arg(ts: in out tsimbols; idp,ida: in id_nom; da: in descripcio;
                    error: out boolean) is
    td: tdescripcio renames ts.td;
    te: texpansio renames ts.te;
    tb: tblocks renames ts.tb;
  ...
  begin
    ...
    while ie > il loop
      if te(ie).prof /= -1 then
        id:= te(ie).id;
        td(id).prof:= te(ie).prof; td(id).d:= te(ie).d; 
