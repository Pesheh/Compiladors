...
package body semantica.c_tipus is
  ...
  procedure ct_e2(nd_e: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out boolean; pos: in out posicio);
  procedure ct_e2_op_rel(nd_e: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out boolean; pos: in out posicio);
  procedure ct_e2_arit(nd_e: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out boolean; pos: in out posicio; op: in operand );
  procedure ct_e2_neg_log(nd_e: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out boolean; pos: in out posicio);
  procedure ct_e2_neg_arit(nd_e: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out boolean; pos: in out posicio);

  procedure ct_e3(nd_e: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out  boolean; pos: in out posicio);
  procedure ct_e3_lit(nd_lit: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out boolean; pos: in out posicio);
  procedure ct_e3_ref(nd_ref: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out boolean; pos: in out posicio);
  ...
  begin
    ...
    case nd_q.q_contingut.tn is
      when nd_id => --R.id
        ...
        if not ERROR then
          p:= new node(nd_rec);
          p.rec_td:= nova_var_const(valor(desc_camp.dcmp), tsb_ent);
          if DEBUG then
      ...
      when nd_lexpr => --R(E) 
        ...
        if not ERROR then
          ...
          p.arry_tb:= nova_var_const(valor(desc_tipus.dt.b), tsb_ent); 
          if DEBUG then
            missatges_ct_debugging("ct_q","invc:"&num_var'Image(p.arry_tb));
          end if;
          desc_tipus:= get(ts,desc_tipus.dt.tcomp);
          p.arry_tw:= nova_var_const(valor(desc_tipus.dt.ocup), tsb_ent);
          if DEBUG then
  ...
  begin
    ...
    if not ERROR then 
      ...
      p.lexpra_tu:= nova_var_const(valor(desc_tipus_idx.dt.lsup - desc_tipus_idx.dt.linf + 1),tsb_ent);
      if DEBUG then
  ...
  procedure ct_e2(nd_e: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out boolean; pos: in out posicio) is
  begin
    if DEBUG then
      missatges_ct_debugging("ct_e2",tnode'Image(nd_e.tn)&"::"
      &operand'Image(nd_e.e2_operand)&"::"
      &tnode'Image(nd_e.e2_opd.tn)&"::"
      &tnode'Image(nd_e.e2_ope.tn));
    end if;
  ...
  procedure ct_e2_op_rel(nd_e: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out boolean; pos: in out posicio) is
    id_tipus1, id_tipus2: id_nom;
    tsb1, tsb2: tipus_subjacent;
    esvar1, esvar2: boolean;
    pos1, pos2: posicio:= (0, 0);
  ...
  begin
    ...
    ct_e2(nd_e.e2_opd, id_tipus2, tsb2, esvar2, pos2);
  ...
  procedure ct_e2_arit(nd_e: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out boolean; pos: in out posicio; op: in operand ) is
    id_tipus1, id_tipus2: id_nom;
    tsb1, tsb2: tipus_subjacent;
    esvar1, esvar2: boolean;
    pos1, pos2: posicio:= (0, 0);
  ...
  procedure ct_e2_neg_log(nd_e: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out boolean; pos: in out posicio) is
    id_tipus: id_nom;
    tsb: tipus_subjacent;
    esvar1: boolean;
  begin
  ...
  procedure ct_e2_neg_arit(nd_e: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out boolean; pos: in out posicio) is
    id_tipus: id_nom;
    tsb: tipus_subjacent;
    esvar1: boolean;
  begin
  ...
  procedure ct_e3(nd_e: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out  boolean; pos: in out posicio) is 
  begin
    if DEBUG then
      missatges_ct_debugging("ct_e3",tnode'Image(nd_e.tn));
    end if;
  ...
  procedure ct_e3_ref(nd_ref: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out boolean; pos: in out posicio) is 
  ...
  procedure ct_e3_lit(nd_lit: in out pnode; id_texpr: out id_nom; tsb_expr: out tipus_subjacent; esvar: out boolean; pos: in out posicio) is 
  begin
    id_texpr:= null_id;
    pos:= nd_lit.lit_pos;
